...
Počet generovaných místností je odvozen od celkové plochy mapy podle vzorce \texttt{room\_count = max(5, width * height / 360)}. Na malých mapách je tak vygenerováno alespoň pět místností, zatímco na velkých mapách počet místností úměrně roste s~plochou. Maximální počet pokusů o~umístění místnosti je stanoven jako \texttt{room\_count * 10}. Každá nová místnost je reprezentována obdélníkem typu \texttt{Rect2}. Její šířka a výška jsou generovány náhodně v~intervalu od 8 do 16 dlaždic. Pozice levého horního rohu místnosti je následně určena tak, aby místnost zůstala uvnitř hranic mapy a zároveň nebyla umístěna přímo na jejím okraji.
...
\subsubsection{Metoda náhodné procházky}
Náhodná procházka je implementována v souboru \texttt{drunken\_generator.gd} (viz zdrojový kód~\ref{lst:drunken}). Generování mapy probíhá opět ve funkci \texttt{generate(width, height)}, která vytváří dvourozměrné pole reprezentující výslednou mapu.
...
Vzhledem k tomu, že cílový počet průchozích dlaždic je určen předem, je výsledné pokrytí mapy pro danou velikost poměrně stabilní. Variabilita generovaných map se proto projevuje především ve tvaru a rozmístění vytvořených prostorů.
...
Implementace buněčných automatů je v souboru \texttt{cellular\_generator.gd} (viz zdrojový kód~\ref{lst:cellular}). První fáze vytvoří počáteční mapu náhodným vyplněním buněk podlahou podle pravděpodobnosti \texttt{FILL\_PROBABILITY}. Okraje mapy zůstávají prázdné, aby se předešlo problémům při následném vyhodnocování sousedství.
...
Metoda bludiště je implementována v souboru \texttt{maze\_generator.gd} (viz zdrojový kód~\ref{lst:maze}) a vychází z algoritmu \textit{recursive backtracker} (DFS se zásobníkem). Vnitřní reprezentace není tvořena přímo dlaždicemi mapy, ale mřížkou abstraktních buněk, z nichž každá obsahuje bitovou masku čtyř stěn.
...
Pro každou kombinaci algoritmu a velikosti mapy je generování provedeno opakovaně v~deseti nezávislých bězích. Tím je zajištěn dostatečný vzorek pro statistické vyhodnocení, zejména pro posouzení průměrných hodnot a variability výsledků.

Měření času generování je realizováno pomocí funkce \texttt{Time.get\_ticks\_usec()}, která poskytuje mikrosekondovou přesnost. Měřen je výhradně čas strávený ve funkci \texttt{generate(width, height)} příslušného algoritmu. Vizualizace mapy, vykreslování dlaždic, umisťování hráče ani další operace nejsou součástí měřeného intervalu. Výsledný čas je převeden na milisekundy.
...
\subsubsection{Drunken Walk (náhodná procházka)}

Algoritmus Drunken Walk se od ostatních metod odlišuje především konstantním pokrytím mapy. Výsledné pokrytí se ve všech testovaných konfiguracích pohybuje v~rozmezí 35{,}0--35{,}1~\%, což přímo odpovídá implementaci, ve které je cílový počet průchozích dlaždic stanoven jako pevný podíl celkové plochy mapy. Variabilita pokrytí je tedy prakticky nulová.

Čas generování naopak vykazuje výraznější variabilitu mezi jednotlivými běhy. Na mapě 20$\times$50 se průměrný čas pohybuje kolem 1{,}5~ms, na největší mapě 150$\times$150 dosahuje přibližně 34{,}7~ms. Rozptyl časů je způsoben stochastickou povahou algoritmu -- náhodná procházka může v~některých bězích rychle pokrýt nové oblasti, zatímco v~jiných se opakovaně vrací do již odkrytých částí mapy. Hodnota 35~\% je přitom nastavitelným parametrem implementace (konstanta v~kódu), nikoliv inherentní vlastností algoritmu -- změnou této konstanty lze dosáhnout libovolného cílového pokrytí.

Drunken Walk tak představuje zajímavý kontrast k~buněčným automatům: zatímco Drunken Walk má předvídatelné pokrytí, ale nepředvídatelný čas (protože procházka může opakovaně navštěvovat již odkryté dlaždice), buněčné automaty mají naopak stabilní čas (vždy proběhne stejný počet simulačních kroků), avšak variabilní pokrytí závislé na~náhodné počáteční konfiguraci.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{images/graphs/drunken_time_by_size.png}
\caption{Drunken Walk -- průměrný čas generování podle velikosti mapy (Generation Time by Map Size)}
\label{fig:drunken_time}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{images/graphs/drunken_coverage_by_size.png}
\caption{Drunken Walk -- průměrné pokrytí podle velikosti mapy (Coverage by Map Size)}
\label{fig:drunken_coverage}
\end{figure}


\subsubsection{Cellular (buněčné automaty)}
...
\caption{Cellular -- průměrný čas generování podle velikosti mapy (Generation Time by Map Size)}
...
\caption{Cellular -- průměrné pokrytí podle velikosti mapy (Coverage by Map Size)}
...
Tato podkapitola porovnává všechny implementované metody z~hlediska času generování, pokrytí mapy a jejich vzájemného vztahu.

\subsubsection{Průměrný čas generování}

Celkový pohled na průměrné časy generování ukazuje výrazné rozdíly mezi jednotlivými metodami. Nejrychlejšími algoritmy jsou BSP a Maze, jejichž průměrné časy na mapě 150$\times$150 dosahují přibližně 24~ms. Rooms se po úpravě algoritmu řadí do podobného rozmezí s~časem přibližně 29{,}5~ms na největší mapě. Drunken Walk dosahuje na této velikosti přibližně 34{,}7~ms. Buněčné automaty jsou řádově nejpomalejší -- na mapě 150$\times$150 je jejich průměrný čas přibližně 175{,}7~ms, tedy přibližně sedmkrát více než u~BSP a Maze.
...
Z~hlediska celkového průměrného pokrytí dosahují nejvyšších hodnot buněčné automaty a BSP, následované algoritmem Rooms. Maze dosahuje stabilního pokrytí kolem 60~\%. Drunken Walk má konstrukčně dané pokrytí přibližně 35~\%.
...
Obrázek~\ref{fig:comp_coverage_scaling} ukazuje vývoj pokrytí v~závislosti na velikosti mapy. BSP, Cellular a Rooms vykazují rostoucí tendenci -- s~větší mapou roste i~podíl průchozího prostoru. U~Rooms je tento nárůst dán škálováním počtu místností s~plochou mapy. Maze má mírně rostoucí, ale velmi stabilní pokrytí. Drunken Walk zůstává konstantní na hodnotě přibližně 35~\%.
...
Obrázek~\ref{fig:comp_tradeoff} vizualizuje vztah mezi průměrným časem generování a průměrným pokrytím pro jednotlivé algoritmy. Ideální algoritmus by se nacházel v~levé horní části grafu, tedy s~nízkým časem a vysokým pokrytím.

BSP a Maze se nacházejí v~oblasti nízkého času a středně vysokého pokrytí. Rooms se po úpravě algoritmu posunul do oblasti vyššího pokrytí za cenu mírně vyššího času. Buněčné automaty dosahují nejvyššího pokrytí, avšak za cenu výrazně vyššího času. Drunken Walk představuje metodu s~konstrukčně daným nižším pokrytím.
...
Pro detailnější pohled na variabilitu výsledků jsou uvedeny boxploty času generování a pokrytí na největší testované mapě 150$\times$150. Na obrázku~\ref{fig:size150_time_box} je patrné, že buněčné automaty vykazují nejmenší relativní rozptyl času (při výrazně vyšším absolutním čase), zatímco Drunken Walk má vyšší variabilitu. Obrázek~\ref{fig:size150_cov_box} ukazuje, že Maze a Drunken Walk mají prakticky nulovou variabilitu pokrytí, zatímco Rooms, BSP a Cellular vykazují určitý rozptyl.
...
Drunken  & 1{,}47  & 4{,}84  & 2{,}92  & 16{,}35  & 34{,}69  \\
...
Drunken  & 35{,}13 & 35{,}01 & 35{,}01 & 35{,}01 & 35{,}00 \\
...
Na základě provedených měření lze algoritmy rozdělit do dvou skupin podle výpočetní náročnosti. Do první skupiny patří BSP, Maze, Rooms a Drunken Walk, jejichž časy generování na mapě 150$\times$150 nepřekračují 35~ms. Buněčné automaty tvoří samostatnou kategorii s~průměrným časem přibližně 175{,}7~ms na této velikosti.

Z~hlediska pokrytí mapy se algoritmy rovněž výrazně liší. BSP, Cellular a Rooms dosahují na velkých mapách pokrytí přes 60~\%, přičemž BSP a Cellular překračují 73~\%. Maze generuje stabilní pokrytí kolem 60--64~\%. Drunken Walk má konstrukčně dané pokrytí přibližně 35~\%.
...
Drunken Walk vytváří organicky tvarované chodby s~předvídatelným pokrytím. Konstantní pokrytí může být výhodou v~situacích, kdy je požadována přesná kontrola nad poměrem průchozího prostoru. Nevýhodou je nižší strukturovanost výsledného prostředí -- vznikají spíše propojené chodby než oddělené místnosti.
...
Při implementaci buněčných automatů bylo nutné řešit problém izolovaných průchozích oblastí. U~ostatních metod je průchodnost mapy zajištěna již principem algoritmu -- explicitní propojování místností u~Rooms a BSP, návštěva všech buněk u~Maze a spojitá procházka u~Drunken Walk.
...
V~teoretické části práce byly popsány základní principy procedurálního generování obsahu, jeho historický vývoj a~přehled vybraných metod včetně jejich výhod a~omezení. Na~základě provedené rešerše bylo vybráno pět algoritmů reprezentujících různé přístupy ke~generování map: náhodné umístění místností, Binary Space Partitioning, náhodná procházka (Drunken Walk), buněčné automaty a~generování bludiště.
...
\begin{lstlisting}[language=GDScript,style=kibasestyle,caption={Generátor náhodné procházky (\texttt{drunken\_generator.gd}).},label={lst:drunken}]
extends RefCounted
