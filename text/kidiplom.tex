...
Nejprve jsou představeny nástroje využité při vývoji aplikace. Následně je popsán samotný návrh systému, především způsob reprezentace dungeonové mapy, struktura generátoru a způsob vizualizace výsledného prostředí.
...
Pro vizualizaci generátoru map byl zvolen herní engine Godot~\cite{godot_engine}. Jedná se o open-source engine určený pro vývoj 2D i 3D her. Poskytuje integrované nástroje pro práci s grafikou, fyzikou, scénami i skriptováním, což umožňuje rychlé vytváření a testování herních prototypů.
...
Součástí aplikace je jednoduché uživatelské rozhraní, které umožňuje výběr konkrétní generovací metody a nastavení základních parametrů mapy, šířky a výšky.
...
Algoritmus BSP vytváří mapy s~dobře definovanými místnostmi a systematickým propojením. Čas generování roste s~velikostí mapy -- od přibližně 0{,}81~ms na mapě 20$\times$50 po 20{,}5~ms na mapě 150$\times$150. Tento nárůst odpovídá rekurzivní povaze algoritmu, který musí rozdělit celý prostor mapy a následně propojit vzniklé místnosti.
...
Algoritmus náhodné procházky se od ostatních metod odlišuje především konstantním pokrytím mapy. Výsledné pokrytí se ve všech testovaných konfiguracích pohybuje v~rozmezí 50{,}0-50{,}1~\%, což přímo odpovídá implementaci, ve které je cílový počet průchozích dlaždic stanoven jako pevný podíl celkové plochy mapy. Variabilita pokrytí je tedy prakticky nulová.
...
Rozptyl časů je způsoben stochastickou povahou algoritmu -- náhodná procházka může v~některých bězích rychle pokrýt nové oblasti, zatímco v~jiných se opakovaně vrací do již odkrytých částí mapy. Hodnota 50~\% je přitom nastavitelným parametrem implementace (konstanta v~kódu), nikoliv inherentní vlastností algoritmu - změnou této konstanty lze dosáhnout libovolného cílového pokrytí.
...
Algoritmus Rooms dosahuje srovnatelného průměru. Drunkard's Walk je výrazně pomalejší s~celkovým průměrem přibližně 20~ms, což je způsobeno opakovaným navracením do~již odkrytých oblastí. Buněčné automaty jsou řádově nejpomalejší -- jejich celkový průměr přesahuje 61~ms, tedy přibližně osmkrát více než u~BSP. Tento rozdíl je způsoben tím, že buněčné automaty v~každém z~pěti simulačních kroků procházejí celou mapu, zatímco ostatní algoritmy pracují pouze s~omezenou podmnožinou buněk. Na~největší testované mapě 150$\times$150 jsou absolutní rozdíly ještě výraznější -- BSP dosahuje přibližně 21~ms, Maze 24~ms, Rooms 25{,}9~ms, Drunkard's Walk 57{,}1~ms, zatímco Cellular automata přibližně 179~ms (viz tabulka~\ref{tab:results_summary}).
...
Na základě provedených měření lze algoritmy rozdělit do tří skupin podle výpočetní náročnosti. Do první skupiny patří BSP, Maze a~Rooms, jejichž časy generování na mapě 150$\times$150 nepřekračují 26~ms. Drunkard's Walk tvoří střední kategorii s~průměrným časem přibližně 57{,}1~ms, který je způsoben opakovaným navracením do již odkrytých oblastí. Buněčné automaty jsou nejpomalejší s~průměrným časem přibližně 178{,}8~ms na této velikosti.
