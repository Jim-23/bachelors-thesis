...
S procedurálním generováním se dnes setkáváme téměř v každé moderní videohře, jeho počátky však sahají již do 70. let 20. století. V tomto období byl výpočetní výkon i dostupná operační paměť velmi omezené. Tyto limity výrazně omezovaly množství ručně vytvářeného obsahu, a vývojáři proto hledali způsoby, jak vytvářet variabilní herní prostředí s minimálními nároky na paměť RAM. Právě v tomto kontextu se začaly prosazovat procedurální přístupy~\cite{Shaker2016}.
...
Ve 2D hrách se nejčastěji setkáváme s metodami založenými na náhodném umísťování místností a jejich následném propojování, s rozdělováním velkého prostoru na menší části nebo s algoritmy založenými na takzvané náhodné procházce. Každý z těchto přístupů má své výhody i omezení a je vhodný pro odlišný typ herního prostředí. Porovnáním efektivity vybraných kombinací těchto algoritmů z~hlediska času generování a spotřeby výpočetních zdrojů se zabývá například práce~\cite{Monaghan2019}.
...
Pro vizualizaci generátoru dungeonových map byl zvolen herní engine Godot~\cite{godot_engine}. Jedná se o open-source engine určený pro vývoj 2D i 3D her. Poskytuje integrované nástroje pro práci s grafikou, fyzikou, scénami i skriptováním, což umožňuje rychlé vytváření a testování herních prototypů.
...
Pro implementaci algoritmů byl použit integrovaný skriptovací jazyk GDScript~\cite{godot_gdscript}, který je syntakticky podobný jazyku Python. GDScript umožňuje přímý přístup k objektům herní scény, komponentám enginu i datovým strukturám, což usnadňuje rychlé prototypování generovacích metod a jejich průběžné ladění přímo v prostředí enginu. Při vývoji podpůrných částí aplikace (ovládání kamery, herní mechaniky) byl jako pomocný nástroj využit GitHub Copilot; samotné generovací algoritmy jsou vlastní implementací na~základě publikovaných algoritmických popisů.
...
V rámci implementace byl využit volně dostupný balíček \textit{2D Pixel Dungeon Asset Pack}~\cite{pixel_dungeon_asset}, jehož autorem je grafik vystupující pod jménem Pixel\_Poem. Tento balíček obsahuje základní sadu grafických prvků určených pro tvorbu dungeonových roguelike her.
...
Výsledky jednotlivých běhů jsou ukládány do souboru \texttt{results.csv}, jehož struktura je podrobněji popsána v~podkapitole věnované metodice měření.
...
Chodba není tvořena pouze jednou dlaždicí, ale má nastavitelnou šířku. V implementaci je výchozí hodnota parametru \texttt{corridor\_width} nastavena na~2. Z~ní se vypočte poloviční rozsah \texttt{half = int(corridor\_width * 0.5) = 1} a~při vykreslování se centrální linie chodby rozšíří o~posun v~rozmezí $-1$ až $+1$, výsledná chodba je tedy široká 3 dlaždice. Při zápisu dlaždic chodby je současně kontrolováno, zda se zapisované souřadnice nacházejí uvnitř hranic mapy, a to pomocí pomocné funkce \texttt{\_in\_bounds()}.
...
Rekurzivní dělení prostoru provádí metoda \texttt{split(remaining, paths, rng)}. Směr řezu je určen deterministicky: pokud je výška oblasti větší nebo rovna šířce (\texttt{size.y >= size.x}), dělí se horizontálně, jinak vertikálně. Pozice řezu je volena náhodně v~rozmezí 35--65\,\% délky dělené dimenze, takže vzniklé podoblasti nemusí být stejně velké. Při každém dělení jsou do~pole \texttt{paths} uloženy středy obou vzniklých potomků, které jsou později použity pro vykreslení chodeb.

Funkce \texttt{split()} vždy vytvoří dva potomky a~poté, pokud je \texttt{remaining~>~0}, zavolá se rekurzivně na~oba potomky s~hodnotou \texttt{remaining~-~1}. 
Při volání \texttt{split(3)} se tak dělení provádí ve~čtyřech úrovních (pro \texttt{remaining} 3, 2, 1 a~0) a~výsledný strom má až $2^4 = 16$ listových uzlů. 
...
Algoritmus začíná ve~středu mapy na~pozici \texttt{width/2}, \texttt{height/2}. Kolem počáteční pozice je ihned vytvořena oblast podlahy o~velikosti $3 \times 3$ dlaždic pomocí funkce \texttt{\_carve\_3x3()}, která slouží jako bezpečný výchozí prostor pro hráče. Cílový počet průchozích dlaždic je stanoven na~35\,\% celkové plochy mapy, tedy \texttt{(width * height * 35) / 100}.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{images/drunkards_map.png}
\caption{Ukázka mapy generované pomocí algoritmu náhodné procházky}
\label{fig:drunkards_map}
\end{figure}
...
Pro každou kombinaci algoritmu a velikosti mapy je generování provedeno opakovaně v~deseti nezávislých bězích, celkem tedy dosáhneme na minimálně 250 záznamů. Tím je zajištěn dostatečný vzorek pro statistické vyhodnocení, zejména pro posouzení průměrných hodnot a variability výsledků.

Měření času generování je realizováno pomocí funkce \texttt{Time.get\_ticks\_usec()}, která poskytuje mikrosekondovou přesnost. 
...
Algoritmus Rooms vykazuje nízké časy generování na menších mapách -- na mapě 20$\times$50 přibližně 0{,}89~ms. S~rostoucí velikostí mapy čas narůstá výrazněji, protože počet generovaných místností je úměrný ploše mapy. Na mapě 150$\times$150 dosahuje průměrný čas přibližně 29{,}5~ms.

Pokrytí mapy průchozím prostorem roste s~velikostí mapy. Na menších mapách (20$\times$50, 70$\times$30) se pohybuje v~rozmezí 34--45~\%, na mapě 100$\times$100 činí přibližně 50~\% a na mapě 150$\times$150 dosahuje přibližně 61~\%. Tento rostoucí trend je dán tím, že počet místností se odvíjí od celkové plochy mapy. Na větších mapách tak vzniká úměrně více místností, které efektivněji vyplňují dostupný prostor.
...
Algoritmus BSP vytváří mapy s~dobře definovanými místnostmi a systematickým propojením. Čas generování roste s~velikostí mapy -- od přibližně 0{,}65~ms na mapě 20$\times$50 po 24{,}2~ms na mapě 150$\times$150. Tento nárůst odpovídá rekurzivní povaze algoritmu, který musí rozdělit celý prostor mapy a následně propojit vzniklé místnosti.

Pokrytí mapy u~BSP roste s~velikostí mapy. Na nejmenší mapě 20$\times$50 činí pokrytí přibližně 27{,}8~\%, zatímco na mapě 150$\times$150 dosahuje hodnot kolem 73{,}8~\%. Tento jev je dán principem algoritmu, který systematicky rozděluje celý dostupný prostor a v~každé vzniklé oblasti vytváří místnost. Na větší mapě tak vzniká více a větších místností, které efektivněji vyplňují celkovou plochu.
...
Algoritmus náhodné procházky se od ostatních metod odlišuje především konstantním pokrytím mapy. Výsledné pokrytí se ve všech testovaných konfiguracích pohybuje v~rozmezí 35{,}0--35{,}1~\%, což přímo odpovídá implementaci, ve které je cílový počet průchozích dlaždic stanoven jako pevný podíl celkové plochy mapy. Variabilita pokrytí je tedy prakticky nulová.

Čas generování naopak vykazuje výraznější variabilitu mezi jednotlivými běhy. 
Na mapě 20$\times$50 se průměrný čas pohybuje kolem 1{,}5~ms, na největší mapě 150$\times$150 dosahuje přibližně 34{,}7~ms. 
Rozptyl časů je způsoben stochastickou povahou algoritmu -- náhodná procházka může v~některých bězích rychle pokrýt nové oblasti, zatímco v~jiných se opakovaně vrací do již odkrytých částí mapy. Hodnota 35~\% je přitom nastavitelným parametrem implementace (konstanta v~kódu), nikoliv inherentní vlastností algoritmu -- změnou této konstanty lze dosáhnout libovolného cílového pokrytí.
...
Buněčné automaty jsou z~testovaných algoritmů jednoznačně nejpomalejší. Na nejmenší mapě 20$\times$50 trvá generování přibližně 7{,}0~ms, na mapě 100$\times$100 již 76{,}8~ms a na největší mapě 150$\times$150 dosahuje průměrný čas přibližně 175{,}7~ms. Tento výrazný nárůst je způsoben tím, že algoritmus v~každém simulačním kroku prochází všechny buňky mapy a vyhodnocuje jejich sousedství.

Pokrytí mapy u~buněčných automatů dosahuje vysokých hodnot a s~rostoucí velikostí mapy mírně roste -- od přibližně 56~\% na mapě 20$\times$50 po 73{,}2~\% na mapě 150$\times$150. Variabilita pokrytí je přitom nezanedbatelná, zejména na menších mapách, kde náhodná inicializace má větší vliv na výsledný tvar průchozích oblastí.
...
Čas generování je u~tohoto algoritmu srovnatelný s~BSP. Na nejmenší mapě činí přibližně 0{,}96~ms, na největší pak 23{,}5~ms. Variabilita času mezi běhy je velmi nízká, protože algoritmus vždy provede stejný počet kroků.
...
Tato podkapitola porovnává všechny implementované metody z~hlediska času generování, pokrytí mapy a~jejich vzájemného vztahu. Pokud není uvedeno jinak, zobrazené hodnoty představují průměry přes všechny testované velikosti map (5~velikostí $\times$ 10~běhů = 50~měření na~algoritmus). Konkrétní hodnoty pro jednotlivé velikosti jsou shrnuty v~tabulce~\ref{tab:results_summary}.

\subsubsection{Průměrný čas generování}

Obrázek~\ref{fig:comp_avg_time} zobrazuje průměrný čas generování jednotlivých algoritmů přes všechny testované velikosti map. Mezi algoritmy jsou patrné výrazné rozdíly. 
Nejrychlejšími metodami jsou BSP a~Maze, jejichž celkový průměr nepřekračuje 10~ms. 
Algoritmus Rooms dosahuje srovnatelného průměru, zatímco Drunkard's Walk je mírně pomalejší. Buněčné automaty jsou řádově nejpomalejší -- jejich celkový průměr přesahuje 60~ms, tedy přibližně sedmkrát více než u~BSP. Tento rozdíl je způsoben tím, že buněčné automaty v~každém z~pěti simulačních kroků procházejí celou mapu, zatímco ostatní algoritmy pracují pouze s~omezenou podmnožinou buněk. Na~největší testované mapě 150$\times$150 jsou absolutní rozdíly ještě výraznější -- BSP a~Maze dosahují přibližně 24~ms, Rooms 29{,}5~ms a~Drunkard's Walk 34{,}7~ms, zatímco Cellular automata přibližně 176~ms (viz tabulka~\ref{tab:results_summary}).
...
Obrázek~\ref{fig:comp_avg_coverage} zobrazuje průměrné pokrytí mapy průchozím prostorem přes všechny testované velikosti. Nejvyššího celkového pokrytí dosahují buněčné automaty, následované algoritmem Maze se~stabilním pokrytím kolem 60~\%. BSP a~Rooms dosahují celkového průměru přibližně 49~\% a~46~\%, přičemž u~obou metod pokrytí výrazně roste s~velikostí mapy (viz obrázek~\ref{fig:comp_coverage_scaling}). Drunkard's Walk má konstrukčně dané pokrytí přibližně 35~\%, které se s~velikostí mapy prakticky nemění, protože algoritmus vždy vyřezá přesně 35~\% celkové plochy.
...
Obrázek~\ref{fig:comp_time_scaling} znázorňuje, jak se čas generování mění s~rostoucím počtem buněk mapy. BSP a Maze vykazují přibližně lineární nárůst času. Rooms roste rovněž přibližně lineárně, avšak s~vyšší konstantou, protože na velkých mapách generuje výrazně více místností. Drunkard's Walk roste rychleji, a to kvůli opakovanému navracení do již odkrytých oblastí. Buněčné automaty vykazují nejstrmější nárůst, protože v~každém simulačním kroku je nutné vyhodnotit všechny buňky mapy.
...
Obrázek~\ref{fig:comp_coverage_scaling} ukazuje vývoj pokrytí v~závislosti na velikosti mapy. BSP, Cellular a Rooms vykazují rostoucí tendenci -- s~větší mapou roste i~podíl průchozího prostoru. U~Rooms je tento nárůst dán škálováním počtu místností s~plochou mapy. Maze má mírně rostoucí, ale velmi stabilní pokrytí. Drunkard's Walk zůstává konstantní na hodnotě přibližně 35~\%.
...
Obrázek~\ref{fig:comp_tradeoff} vizualizuje vztah mezi průměrným časem generování a~průměrným pokrytím pro jednotlivé algoritmy (oba průměry přes všechny testované velikosti map). Ideální algoritmus by se nacházel v~levé horní části grafu, tedy s~nízkým časem a~vysokým pokrytím.

BSP a~Maze se nacházejí v~oblasti nízkého času a~středně vysokého pokrytí, přičemž BSP dosahuje vyššího pokrytí zejména na~velkých mapách. Rooms nabízí srovnatelný čas s~mírně nižším celkovým pokrytím. Buněčné automaty dosahují nejvyššího pokrytí ze~všech metod, avšak za~cenu výrazně vyššího výpočetního času. Drunkard's Walk představuje metodu s~nejnižším, ale předvídatelným pokrytím a~středně nízkým časem generování.
...
Pro detailnější pohled na variabilitu výsledků jsou uvedeny boxploty času generování a pokrytí na největší testované mapě 150$\times$150. Na obrázku~\ref{fig:size150_time_box} je patrné, že buněčné automaty vykazují nejmenší relativní rozptyl času (při výrazně vyšším absolutním čase), zatímco Drunkard's Walk má vyšší variabilitu. Obrázek~\ref{fig:size150_cov_box} ukazuje, že Maze a Drunkard's Walk mají prakticky nulovou variabilitu pokrytí, zatímco Rooms, BSP a Cellular vykazují určitý rozptyl.
...
\hline
Rooms    & 0{,}89  & 4{,}58  & 1{,}49  & 10{,}67  & 29{,}54  \\
BSP      & 0{,}65  & 2{,}84  & 1{,}46  & 9{,}59   & 24{,}23  \\
Drunkard's  & 1{,}47  & 4{,}84  & 2{,}92  & 16{,}35  & 34{,}69  \\
Cellular & 6{,}99  & 26{,}76 & 15{,}20 & 76{,}80  & 175{,}74 \\
Maze     & 0{,}96  & 3{,}64  & 1{,}95  & 10{,}06  & 23{,}51  \\
\hline
\multicolumn{6}{|c|}{\textit{Průměrné pokrytí [\%]}} \\
\hline
Rooms    & 44{,}82 & 40{,}17 & 33{,}91 & 50{,}23 & 60{,}68 \\
BSP      & 27{,}82 & 46{,}86 & 34{,}76 & 63{,}60 & 73{,}77 \\
Drunkard's  & 35{,}13 & 35{,}01 & 35{,}01 & 35{,}01 & 35{,}00 \\
Cellular & 56{,}07 & 66{,}04 & 60{,}90 & 69{,}38 & 73{,}23 \\
...
Na základě provedených měření lze algoritmy rozdělit do dvou skupin podle výpočetní náročnosti. Do první skupiny patří BSP, Maze, Rooms a Drunkard's Walk, jejichž časy generování na mapě 150$\times$150 nepřekračují 35~ms. Buněčné automaty tvoří samostatnou kategorii s~průměrným časem přibližně 175{,}7~ms na této velikosti.

Z~hlediska pokrytí mapy se algoritmy rovněž výrazně liší. BSP, Cellular a Rooms dosahují na velkých mapách pokrytí přes 60~\%, přičemž BSP a Cellular překračují 73~\%. Maze generuje stabilní pokrytí kolem 60--64~\%. Drunkard's Walk má konstrukčně dané pokrytí přibližně 35~\%.
...
Algoritmus BSP nabízí dobrou rovnováhu mezi rychlostí generování, pokrytím mapy a strukturovaností výsledku. Generované mapy obsahují jasně oddělené místnosti propojené chodbami, což odpovídá tradičnímu pojetí dungeonových prostředí. Nevýhodou je poměrně pravidelný vzhled výsledných map, který může působit uniformně.

Rooms po úpravě škálování počtu místností dosahuje na velkých mapách výrazně vyššího pokrytí než bez této úpravy a zároveň nabízí přirozenější rozložení prostoru díky náhodnému umisťování. Variabilita mezi běhy je u~tohoto algoritmu vyšší, což zvyšuje znovuhratelnost, ale zároveň snižuje předvídatelnost výsledku.
...
Drunkard's Walk vytváří organicky tvarované chodby s~předvídatelným pokrytím. Konstantní pokrytí může být výhodou v~situacích, kdy je požadována přesná kontrola nad poměrem průchozího prostoru. Nevýhodou je nižší strukturovanost výsledného prostředí oproti buněčných automatů.

Algoritmus Maze byl do implementace zahrnut primárně pro demonstraci odlišného přístupu k~procedurálnímu generování. Generuje dokonalé bludiště s~nulovou variabilitou pokrytí a velmi nízkou variabilitou času. Z~hlediska dungeonových map je však méně vhodný, protože vytváří převážně úzké chodby bez větších otevřených prostor.

\subsection{Zkušenosti z implementace}

Při implementaci buněčných automatů bylo nutné řešit problém izolovaných průchozích oblastí. U~ostatních metod je průchodnost mapy zajištěna již principem algoritmu -- explicitní propojování místností u~Rooms a BSP, návštěva všech buněk u~Maze a spojitá procházka u~Drunkard's Walk.

Úprava algoritmu Rooms, při které byl pevný počet místností nahrazen škálováním úměrným ploše mapy, vedla k~výraznému zlepšení pokrytí na velkých mapách. Tato zkušenost ukazuje, že i~drobná změna parametrizace může zásadně ovlivnit výsledné vlastnosti generovaného prostředí.
...
Všech pět algoritmů bylo implementováno v~herním enginu Godot s~využitím skriptovacího jazyka GDScript. Systém byl navržen modulárně s~jednotným rozhraním, které umožňuje snadné přidávání dalších generovacích metod. Součástí aplikace je automatizovaný benchmarkový režim, který zajišťuje objektivní a~opakovatelné měření výkonu jednotlivých algoritmů.

Experimentální měření bylo provedeno na~pěti velikostech map v~deseti nezávislých bězích pro každou konfiguraci. Výsledky ukázaly výrazné rozdíly mezi metodami. Algoritmy BSP a~Maze se ukázaly jako nejrychlejší s~časy kolem 24~ms na~mapě 150$\times$150. Algoritmus Rooms dosahuje po~úpravě škálování počtu místností pokrytí přibližně 61~\% na~největší mapě. Buněčné automaty generují mapy s~nejvyšším pokrytím (přes 73~\%), avšak s~výrazně vyšším výpočetním časem přibližně 176~ms. Drunkard's Walk poskytuje konstrukčně dané pokrytí přibližně 35~\%.
...
Experimental measurements were conducted on five map sizes with ten independent runs per configuration. The results revealed significant differences between the methods. BSP and Maze proved to be the fastest algorithms, with generation times around 24~ms on a~150$\times$150 map. The Rooms algorithm, after adjusting its room count scaling, achieves approximately 61~\% coverage on the largest map. Cellular automata generate maps with the highest coverage (over 73~\%), but at the cost of significantly higher computation time of approximately 176~ms. Drunkard's walk provides a~structurally fixed coverage of approximately 35~\%.
...
\section{Zkratky}
\begin{description}
\item[2D] Two-Dimensional -- dvourozměrný
\item[3D] Three-Dimensional -- trojrozměrný
\item[ASCII] American Standard Code for Information Interchange -- americký standardní kód pro výměnu informací
\item[BSP] Binary Space Partitioning -- binární dělení prostoru
\item[CSV] Comma-Separated Values -- hodnoty oddělené čárkou
\item[DFS] Depth-First Search -- prohledávání do hloubky
\item[PCG] Procedural Content Generation -- procedurální generování obsahu
\item[RAM] Random Access Memory -- paměť s náhodným přístupem
\end{description}
...
\item[\texttt{src/}] \hfill \\
  Kompletní zdrojové soubory projektu a
