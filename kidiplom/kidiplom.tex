...
%%%  Copyright (C) 2012 Jesse Sadowý, <rotter.martinos@gmail.com>
...
\keywords{procedurální generování; dungeon; 2D hry; Godot; herní engine}
\keywords[english]{procedural generation; dungeon; 2D games; Godot; game engine}
...
\thanks{Děkuji vedoucímu práce, panu doc. RNDr. Janu Konečnému, Ph.D. za vedení a cenné rady. Dále bych rád poděkoval své rodině a přátelům za podporu během studia.}
...
Tato kapitola se věnuje uvedení do problematiky procedurálního generování obsahu v počítačových hrách.
Je zde popsána motivace práce, její hlavní cíl a stručný přehled struktury následujících kapitol.

\section{Procedurální generování obsahu}

\subsection{Základní vymezení pojmu}

Tato podkapitola vysvětluje pojem procedurální generování obsahu a jeho obecné principy
v kontextu digitálních her a interaktivních aplikací.

\subsection{Přínosy a omezení procedurálního přístupu}

Podkapitola se zaměřuje na výhody a nevýhody procedurálního generování,
zejména z pohledu variability obsahu, náročnosti vývoje a kontroly nad výsledkem.

\subsection{Využití v herním průmyslu}

Zde jsou stručně uvedeny příklady her, které procedurální generování využívají,
a způsoby, jakými je tento přístup aplikován v praxi.

\section{Dungeony ve 2D hrách}

\subsection{Charakteristické vlastnosti dungeonů}

Tato část popisuje typické rysy dungeonových map,
jako jsou místnosti, chodby, konektivita a herní čitelnost.

\subsection{Základní stavební prvky dungeonových map}

Podkapitola se věnuje jednotlivým prvkům,
ze kterých jsou dungeony nejčastěji tvořeny,
a jejich významu pro herní zážitek.
...
Tato podkapitola stručně představuje herní engine Godot
a odůvodňuje jeho volbu pro praktickou část práce.

\subsection{Skriptovací jazyk GDScript}

Zde je popsán jazyk GDScript a jeho role při implementaci generovacích algoritmů.

\subsection{Práce s dlaždicovou mapou}

Podkapitola se zaměřuje na způsob reprezentace herní mapy pomocí dlaždic
a nástroje poskytované enginem Godot.

\section{Návrh řešení}

\subsection{Datová reprezentace mapy}

Tato část popisuje způsob ukládání a zpracování mapy
pro účely procedurálního generování.

\subsection{Struktura generátoru}

Zde je popsán návrh architektury generátoru
a oddělení logiky generování od vykreslování.

\subsection{Obecný průběh generování}

Podkapitola shrnuje základní kroky,
které jsou společné pro implementované generovací metody.

\section{Implementace generovacích metod}

\subsection{Metoda náhodného umístění místností}

\subsubsection{Princip metody}

\subsubsection{Postup generování}

\subsubsection{Propojování místností chodbami}

\subsubsection{Ukázky generovaných map}

\subsection{Metoda Binary Space Partitioning}

\subsubsection{Princip rozdělování prostoru}

\subsubsection{Postup generování mapy}

\subsubsection{Výsledné struktury dungeonů}
...
\subsubsection{Použitá pravidla transformace}

\subsubsection{Charakter výsledných map}

\subsubsection{Ukázky generovaných map}

\section{Vyhodnocení a porovnání metod}

\subsection{Zvolená kritéria hodnocení}

Tato podkapitola definuje kritéria,
na jejichž základě jsou jednotlivé metody porovnávány.

\subsection{Srovnání výsledků}

Zde je provedeno porovnání implementovaných metod
z hlediska struktury map, variability a výpočetní náročnosti.

\subsection{Zhodnocení dosažených výsledků}

Podkapitola shrnuje pozorované rozdíly mezi metodami
a jejich vhodnost pro různé typy her.

\section{Diskuze}

\subsection{Zkušenosti z implementace}

Tato část reflektuje praktické zkušenosti získané během vývoje
a identifikuje problematická místa řešení.

\subsection{Možnosti dalšího rozšíření}

Zde jsou nastíněny možné směry dalšího vývoje
a rozšíření implementovaného řešení.

\section{Závěr}

Tato kapitola shrnuje dosažené cíle práce,
hodnotí splnění zadání a uzavírá celý text.
...
\subsection{Odkaz na repozitář projektu}
...
program & \vtop{\hbox{\strut itk}\hbox{\strut ainfvs}} & Specifikuje studijní program/obor (specializaci):\newline
\begin{description}
...
\subsection{Definice procedurálního generování}
\subsection{Výhody a nevýhody procedurálního generování}
\subsection{Využití procedurálního generování ve hrách}
