...
Procedurální generování obsahu lze rozdělit podle několika hledisek. Jedním z nich je systém reaguje na hráče. V případě neadaptivního (statického) generování je obsah vytvářen bez ohledu na chování hráče a jeho průběh hrou. Všechny generované struktury vycházejí ze stejných pravidel.
...
Plně automatická generace naopak probíhá bez zásahu člověka během samotného procesu vytváření obsahu. Algoritmus pracuje pouze na základě vstupních parametrů a předem definovaných pravidel. Tento způsob je typický zejména pro roguelike hry a další tituly, kde je cílem vysoká různost jednotlivých průchodů.
...
Herní prostředí v Beneath Apple Manor bylo generováno algoritmicky při každém novém spuštění hry. Tento přístup zajistil, že rozložení místností, chodeb a jednotlivých herních prvků či nepřátel nebolo nikdy totožné, čímž se výrazně zvětšila znovuhratelnost. Cílem hry bylo získat zlaté jablko a progres bylo možné ukládat, takže při smrti hráče se načetla uložená úroveň znovu s tím, že hráč přišel o nějaké atributy.
...
\subsubsection{Rogue}
Za zmínku stojí i herní titul s názvem Rogue, který si oproti výše zmíněnému Beneath Apple Manor získal výrazně větší popularitu a měl zásadní vliv na další vývoj herního designu. Právě podle této hry vznikl podžánr označovaný jako roguelike, jenž z jejích principů vychází dodnes.
...
Rogue je obvykle popisováno jako jedna z prvních roguelike adventur, která výrazně zpopularizovala princip permanentní smrti (permadeath). Postup hrou nebylo možné ukládat a jakmile hráč zemřel, musel začít zcela od začátku. Hráči tak zůstávaly pouze nabyté zkušenosti a znalost herních mechanik. Smrt tedy nepředstavovala jen neúspěch, ale i součást učení se systému hry.
...
Ve 2D hrách se nejčasteji setkáváme s metodami založenými na náhodném umísťováním místností a následném propojování, s rozdělováním velkého prostoru na menší a menší části či s algoritmy založenými na takzvané náhodné procházce. Každý z těchto přístupů má své výhody i omezení a je vhodný pro odlišný typ herního prostředí.

Cílem této podkapitoly je stručně představit princip fungování vybraných metod, jejich základní charakteristicky a typické oblasti využití. Detailnější rozpracování a samotná implementace vybraných algoritmů bude popsána v následujích kapitolách.

\subsubsection{Generování pomocí náhodného umístění místností a chodeb}
Jak již bylo zmíněno výše, generování pomocní náhodného umístění místností a chodeb je jedno z nejpoužívanějších a zároveň nejjednodušší na implementaci.

Hlavní princip metody spočívá v tom, že se v rámci mapy nejprve generují obdelníkové nebo čtvercové struktury, tedy místnosti, různých velikostí. Při generování se především kontroluje, aby následující místnost nepřekrývala již nějakou existující. Výsledkem je tedy množina místností reprezentována například jejich souřadnicemi a rozměry. Následně jsou jednotlivé místnosti propojeny pomocí chodeb- Ty jsou většinou generovány jako jednoduché horizontální a vertikální průchody spojující středy dvou místností nebo jejich nejbližší body. Poměrně běžným řešením je také tzv. L-shaped propojení, kdy jsou dvě místosti spojeny pomocí dvou kolmých chodeb, které dohromady vytvářejí tvar písmene L.

Pro zajištění průchodnosti mapy a toho, že všechny místnosti jsou nějakým způsobem dosažitelné, se často využívají algoritmy z oblasti grafů. Jedním z typických řešení je použití minimální kostry grafu (Minimum Spanning Tree), která zaručuje propojení všech místností při minimálním počtu chodeb.
...
Výhodou této mětody je především jeji jednoduchost a relativně dobrá kontrola nad výslednou strukturou mapy. Generované dungeony obvykle obsahují přehledně oddělené místnosti. Nevýhodou však může být menší variabilita, jelikož místnosti mívají často pravidelný tvar obdelníků a čtverců a mohou tedy působit velmi uniformě a uměle. Navíc se můžeme setkat s méně efektivnějším využitím dostupného prostoru mapy, kdy mezi jednotlivými místnostmi zůstávají větší nevyužité oblasti. Dalším problémem můýe být způsob propojování. Při jednodušších implementacích může docházet ke kolizím chodeb nebo jejich nevhodnému křížení. V některých případech vznikají příliš dlouhé chodby, které snižují dynamiku pohybu hráče a v prostředí.

Z těcho důvodů bývá metoda kombinována s dalšími postupy nebo doplněna o dodatečná pravidla, která limituji velikosti a vzdálenosti místností, délku chodeb nebo celkové rozložení prostoru. \cite{Linden2014,Shaker2016}
%#TODO add diagram
...
Tato kapitola se zabývá návrhem systému pro generování dungeonových map a technologiemi použitými při implementaci vytvořeného prototypu aplikace. Navržený systém slouží k demonstraci a porovnání vybraných metod procedurálního generování popsaných v předchozí kapitole.

Nejprve jsou představeny nástroje využité při vývoji aplikace. Následně je popsán návrh systému generování, především způsob reprezentace dungeonové mapy, struktura generátoru a způsob vizualizace generovaného prostředí.

\subsection{Použité technologie}

\subsubsection{Herní engine Godot}

Pro implementaci generátoru dungeonových map byl zvolen herní engine Godot. Jedná se o open-source herní engine určený pro vývoj 2D i 3D herních aplikací. Engine poskytuje integrované nástroje pro práci s grafikou, fyzikou, scénami i skriptováním, což umožňuje rychlé vytváření herních prototypů.

Významnou vlastností Godotu je jeho podpora dlaždicových map (TileMap), které jsou často využívány při vývoji 2D her, proto se engine primárně používá právě pro 2D hry. Tento způsob reprezentace prostředí je zároveň vhodný pro implementaci algoritmů procedurálního generování, protože umožňuje jednoduchou manipulaci s jednotlivými buňkami mapy.

Engine zároveň umožňuje snadné spouštění a testování různých algoritmů generování v jednom prostředí, což je výhodné při jejich porovnávání.
...
Pro implementaci generovacích algoritmů byl použit skriptovací jazyk GDScript, který je součástí herního enginu Godot. Jazyk je syntakticky podobný jazyku Python a je navržen tak, aby byl snadno čitelný a dobře integrovaný v enginu.
...
Další výhodou je možnost rychlého prototypování a úprav algoritmů během vývoje. Implementované generátory tak mohou být jednoduše spouštěny a testovány přímo v prostředí enginu, což usnadňuje jejich ladění a následné porovnávání výsledků jednotlivých metod. \cite{godot_gdscript}
...
V implementaci jsou rozlišeny tři základní stavy buněk: prázdný prostor, podlaha a stěna. Tyto stavy jsou reprezentovány číselnými hodnotami 0, 1 a 2, které odpovídají prázdné buňce, průchozí podlaze a stěně, které následně slouží jako základ pro vykreslení odpovídajících dlaždic.
...
Systém generování dungeonových map byl navržen modulárním způsobem, aby bylo možné snadno implementovat a porovnávat více metod generování. Každý generovací algoritmus je realizován v samostatném skriptu a poskytuje jednotné rozhraní ve formě funkce \texttt{generate(width, height)}, která na základě zadaných rozměrů mapy vytváří a vrací výslednou datovou reprezentaci mapy.

Tento přístup umožňuje implementovat různé generovací metody bez nutnosti měnit ostatní části systému. V rámci vytvořeného prototypu byly tímto způsobem implementovány například metody generování pomocí náhodného umístění místností, Binary Space Partitioning, buněčných automatů, náhodné procházky a generování bludiště.

Nad jednotlivými generátory stojí hlavní řídicí skript aplikace, který zajišťuje výběr konkrétní generovací metody na základě uživatelského vstupu, spuštění procesu generování a následné zpracování výsledné mapy. Po dokončení generování je mapa předána části systému zodpovědné za její vizualizaci.

Významnou vlastností navržené struktury je oddělení logiky generování od vizualizační vrstvy. Generátory pracují pouze s datovou reprezentací mapy, zatímco samotné vykreslení výsledku v herním prostředí je řešeno samostatně pomocí komponent herního enginu.
...
Součástí prototypu je také jednoduchá reprezentace hráče, která slouží především pro ověření průchodnosti mapy. Hráč je po vygenerování umístěn na vhodnou průchozí pozici. V implementaci je přitom upřednostněna oblast, která poskytuje dostatek volného prostoru v bezprostředním okolí, aby nedocházelo ke kolizi se stěnami bezprostředně po vytvoření mapy.
